...
double DcfdPixie::cfd(const Trace *trace, int i) {
    return FF(trace, i)*(1-w) - FF(trace, i-D);
}

double DcfdPixie::FF(const Trace *trace, int i) {
    double ff = 0;
...
pair<int,int> DcfdPixie::findValidRange(const Trace *trace) {
    double minVal = 1e9;
    double maxVal = -1e9;
...
double DcfdPixie::getPhase(const Trace *trace) {
    double prevCfd = 0;
    double cfdVal = 0;
    int breakPoint = -1;
